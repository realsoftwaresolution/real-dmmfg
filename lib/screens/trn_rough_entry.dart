...
//     final provider = context.read<RoughProvider>();
//     // ✅ dd/MM/yyyy → yyyy-MM-dd convert karo before sending to API
//     String _toIso(String? val) {
...
//     merged['roughDate'] = _toIso(merged['roughDate']?.toString());
//     merged['dueDate']   = _toIso(merged['dueDate']?.toString());
//
//     bool success;
//     if (_isEditMode && _selectedRough != null) {
//       success = await provider.updateRough(
...
//                 data:         _charniEntryData,
//                 columns:      _charniEntryKeys,
//                 title:        'CHARNI  |  Total Wt: ${_totalWt.toStringAsFixed(3)}',
//                 theme:        theme,
...
class _TrnRoughEntryState extends State<TrnRoughEntry> {
  ...
  List<ErpColumnConfig> get _extraColumns => [
    ...
    ErpColumnConfig(key: 'inv',              label: 'INV'),

    ErpColumnConfig(key: 'roughTypeCode',    label: 'TYPE'),
    ErpColumnConfig(key: 'articalCode',      label: 'ARTICAL'),
    ErpColumnConfig(key: 'jangadCharniCode', label: 'JAN. CHARNI'),

    // Rates
    ErpColumnConfig(key: 'exRate',      label: 'EX RATE'),
    ErpColumnConfig(key: 'rateDollar',  label: 'RATE \$'),
    ErpColumnConfig(key: 'rateRs',      label: 'RATE RS'),

    // Expenses
    ErpColumnConfig(key: 'rgExpPer',    label: 'RG EXP %'),
    ErpColumnConfig(key: 'poExpPer',    label: 'PO EXP %'),

    // Size
    ErpColumnConfig(key: 'rgSize',      label: 'RG SIZE'),
    ErpColumnConfig(key: 'poSize',      label: 'PO SIZE'),
    ErpColumnConfig(key: 'lsPer',       label: 'LS %'),

    // Other
    ErpColumnConfig(key: 'mainCutNo',   label: 'MAIN CUT NO'),
    ErpColumnConfig(key: 'dueDay',      label: 'DUE DAY'),
    ErpColumnConfig(key: 'dueDate',     label: 'DUE DATE', isDate: true),

    ErpColumnConfig(key: 'remarks',     label: 'REMARKS'),

    // Entry Fields
    ErpColumnConfig(key: 'charniCode',  label: 'CHARNI'),
    ErpColumnConfig(key: 'charniPc',    label: 'PC'),
    ErpColumnConfig(key: 'charniWt',    label: 'WT'),

    // Process Days
    ErpColumnConfig(key: 'stockTypeCode', label: 'STOCK TYPE'),
    ErpColumnConfig(key: 'entryDays',     label: 'DAYS'),
  ...
  List<List<ErpFieldConfig>> _formRows(
      PartyProvider p, RoughTypeProvider rt, ArticleProvider ar,
      JangadCharaniProvider jc, StockTypeProvider st, CharniProvider ch,
      ) =>
      [
        // SECTION 0: BASIC INFORMATION
        [
          ErpFieldConfig(key: 'roughDate', label: 'DATE', type: ErpFieldType.date,
              readOnly: true,
              required: true, flex: 1, sectionTitle: 'BASIC INFORMATION', sectionIndex: 0),
          ErpFieldConfig(key: 'roughMstID', label: 'ID', type: ErpFieldType.number,
              readOnly: true, flex: 1, sectionIndex: 0),
          ErpFieldConfig(key: 'jno',     label: 'JNO',  type: ErpFieldType.number, flex: 1, sectionIndex: 0),
        ...
        [
          ...
          ErpFieldConfig(key: 'site',    label: 'SITE', flex: 1, sectionIndex: 0),
          ErpFieldConfig(key: 'inv',     label: 'INV',  flex: 1, sectionIndex: 0),
          ErpFieldConfig(key: 'partyCode', label: 'PARTY', type: ErpFieldType.dropdown,
              dropdownItems: p.list.where((e) => e.active == true)
                  .map((e) => ErpDropdownItem(label: e.partyName ?? '', value: e.partyCode?.toString() ?? '')).toList(),
              sectionIndex: 0, flex: 1),
        ],
        [

          ErpFieldConfig(key: 'roughTypeCode', label: 'TYPE', type: ErpFieldType.dropdown,
              dropdownItems: rt.roughTypes.where((e) => e.active == true)
                  .map((e) => ErpDropdownItem(label: e.roughTypeName ?? '', value: e.roughTypeCode?.toString() ?? '')).toList(),
              sectionIndex: 0, flex: 2),
          ErpFieldConfig(key: 'articalCode', label: 'ARTICAL', type: ErpFieldType.dropdown,
              dropdownItems: ar.list.where((e) => e.active == true)
                  .map((e) => ErpDropdownItem(label: e.articalName ?? '', value: e.articalCode?.toString() ?? '')).toList(),
              sectionIndex: 0, flex: 2),
          ErpFieldConfig(key: 'jangadCharniCode', label: 'JAN. CHARNI', type: ErpFieldType.dropdown,
              dropdownItems: jc.list.where((e) => e.active == true)
                  .map((e) => ErpDropdownItem(label: e.jangadCharniName ?? '', value: e.jangadCharniCode?.toString() ?? '')).toList(),
              sectionIndex: 0, flex: 2),
        ],
        // [
        //
        ...
        // SECTION 1: RATES & AMOUNTS
        [
          ErpFieldConfig(key: 'exRate',    label: 'EX RATE', type: ErpFieldType.amount,
              flex: 1, sectionTitle: 'RATES & AMOUNTS', sectionIndex: 1),
          ErpFieldConfig(key: 'rateDollar', label: 'RATE \$', type: ErpFieldType.amount, flex: 1, sectionIndex: 1),
          ErpFieldConfig(key: 'amtDollar', label: 'AMT \$', type: ErpFieldType.amount,
              readOnly: true, flex: 1, sectionIndex: 1, helperText: 'Rate\$ × Tot.Wt'),
        ],
        [
          ErpFieldConfig(key: 'rateRs', label: 'RATE RS', type: ErpFieldType.amount,
              readOnly: true, flex: 1, sectionIndex: 1, helperText: 'ExRate × Rate\$'),
          ErpFieldConfig(key: 'amtRs', label: 'AMT RS', type: ErpFieldType.amount,
              readOnly: true, flex: 1, sectionIndex: 1, helperText: 'RateRs × Tot.Wt'),
        ],

        // SECTION 2: EXPENSES & SIZE
        [
          ErpFieldConfig(key: 'rgExpPer', label: 'RG EXP %', type: ErpFieldType.amount,
              flex: 1, sectionTitle: 'EXPENSES & SIZE', sectionIndex: 2),
          ErpFieldConfig(key: 'poExpPer', label: 'PO EXP %', type: ErpFieldType.amount, flex: 1, sectionIndex: 2),
        ],
        [
          ErpFieldConfig(key: 'rgSize', label: 'RG SIZE', type: ErpFieldType.amount, flex: 1, sectionIndex: 2),
          ErpFieldConfig(key: 'poSize', label: 'PO SIZE', type: ErpFieldType.amount, flex: 1, sectionIndex: 2),
          ErpFieldConfig(key: 'lsPer', label: 'LS %', type: ErpFieldType.amount, flex: 1, sectionIndex: 2),
        ...
        // SECTION 3: OTHER
        [
          ErpFieldConfig(key: 'mainCutNo', label: 'MAIN CUT NO',
              flex: 2, sectionTitle: 'OTHER', sectionIndex: 3),
          ErpFieldConfig(key: 'dueDay', label: 'DUE DAY', type: ErpFieldType.number, flex: 1, sectionIndex: 3),
          ErpFieldConfig(key: 'dueDate', label: 'DUE DATE', type: ErpFieldType.date,
              readOnly: true, flex: 2, sectionIndex: 3, helperText: 'Date + Due Day'),
        ],
        [
          ErpFieldConfig(key: 'remarks', label: 'REMARKS', maxLines: 2, flex: 1, sectionIndex: 3),
        ],

        // SECTION 4: CHARNI ENTRY
        // isEntryField: true → ErpForm last entry field ke Enter pe onEntryAdd(4) call karega
        [
          ErpFieldConfig(key: 'charniCode', label: 'CHARNI', type: ErpFieldType.dropdown,
              dropdownItems: ch.list.where((e) => e.active == true)
                  .map((e) => ErpDropdownItem(label: e.charniName ?? '', value: e.charniCode?.toString() ?? '')).toList(),
              flex: 2, sectionTitle: 'CHARNI ENTRY', sectionIndex: 4,
              isEntryField: true, isEntryRequired: true),
          ErpFieldConfig(key: 'charniPc', label: 'PC', type: ErpFieldType.number,
              flex: 1, sectionIndex: 4, isEntryField: true),
          ErpFieldConfig(key: 'charniWt', label: 'WT', type: ErpFieldType.amount,
              flex: 1, sectionIndex: 4, isEntryField: true, isEntryRequired: true,            showAddButton: true
          ),
        ],

        // SECTION 5: PROCESS DAYS ENTRY
        [
          ErpFieldConfig(key: 'stockTypeCode', label: 'STOCK TYPE', type: ErpFieldType.dropdown,
  ...
  void _recalcDueDate() {
    try {
      final base = DateFormat('dd/MM/yyyy').parse(_formValues['roughDate'] ?? '');
      final days = int.tryParse(_formValues['dueDay'] ?? '') ?? 0;
      _formValues['dueDate'] = DateFormat('dd/MM/yyyy').format(base.add(Duration(days: days)));
    } catch (_) {}
  }
  ...
  void _recalcCharniPer() {
    ...
    _charniRows = _charniRows.asMap().entries.map((e) {
      ...
      return RoughDetModel(srno: e.value.srno, charniCode: e.value.charniCode,
          charniName: e.value.charniName, pc: e.value.pc, wt: e.value.wt, per: per);
    }).toList();
  }

  void _pushCalcToForm() {
    _erpFormKey.currentState?.updateFieldValue('rateRs',    _formValues['rateRs']);
    _erpFormKey.currentState?.updateFieldValue('amtDollar', _formValues['amtDollar']);
    _erpFormKey.currentState?.updateFieldValue('amtRs',     _formValues['amtRs']);
    _erpFormKey.currentState?.updateFieldValue('dueDate',   _formValues['dueDate']);
  }

  // ── Lookup helpers ─────────────────────────────────────────────────────────
  String _charniName(String? code) {
    if (code == null || code.isEmpty) return '';
    return context.read<CharniProvider>().list
        .firstWhere((e) => e.charniCode?.toString() == code, orElse: () => CharniModel())
        .charniName ?? '';
  }

  String _stockTypeName(String? code) {
    if (code == null || code.isEmpty) return '';
    return context.read<StockTypeProvider>().list
        .firstWhere((e) => e.stockTypeCode?.toString() == code, orElse: () => StockTypeModel())
        .stockTypeName ?? '';
  }
  ...
  void _addCharniEntry() {
    ...
    setState(() {
      if (_editingCharniIndex != null) {
        ...
        _charniRows[idx] = RoughDetModel(srno: idx + 1,
            charniCode: int.tryParse(code), charniName: name, pc: pc, wt: wt, per: 0);
        _editingCharniIndex = null;
      } else {
        // ── ADD new row ─────────────────────────────────────────────────
        final srno = _charniRows.length + 1;
        _charniRows.add(RoughDetModel(srno: srno,
            charniCode: int.tryParse(code), charniName: name, pc: pc, wt: wt, per: 0));
      }
      _recalcCharniPer();
      _syncCharniGrid();
    ...
    _pushCalcToForm();
    _clearFields(['charniCode', 'charniPc', 'charniWt'], focusFirst: 'charniCode');
  }
  ...
  void _editCharniRow(int idx) {
    ...
    _erpFormKey.currentState?.updateFieldValue('charniCode', row.charniCode?.toString());
    _erpFormKey.currentState?.updateFieldValue('charniPc',   row.pc?.toString());
    _erpFormKey.currentState?.updateFieldValue('charniWt',   row.wt?.toStringAsFixed(2));
  ...
  void _deleteCharniRow(int idx) {
    setState(() {
      ...
      _charniRows = _charniRows.asMap().entries
          .map((e) => RoughDetModel(srno: e.key + 1, charniCode: e.value.charniCode,
          charniName: e.value.charniName, pc: e.value.pc, wt: e.value.wt, per: e.value.per))
          .toList();
      _recalcCharniPer();
      _syncCharniGrid();
      _recalcRates();
  ...
  void _addProcessDaysEntry() {
    ...
    setState(() {
      if (_editingProcessDaysIndex != null) {
        ...
        _processDaysRows[idx] = RoughProcessDaysModel(srno: idx + 1,
            stockTypeCode: int.tryParse(code), stockTypeName: name, days: days);
        _editingProcessDaysIndex = null;
      } else {
        final srno = _processDaysRows.length + 1;
        _processDaysRows.add(RoughProcessDaysModel(srno: srno,
            stockTypeCode: int.tryParse(code), stockTypeName: name, days: days));
      }
  ...
  void _editProcessDaysRow(int idx) {
    ...
    _erpFormKey.currentState?.updateFieldValue('stockTypeCode', row.stockTypeCode?.toString());
    _erpFormKey.currentState?.updateFieldValue('entryDays',     row.days?.toStringAsFixed(0));
  ...
  void _deleteProcessDaysRow(int idx) {
    setState(() {
      ...
      _processDaysRows = _processDaysRows.asMap().entries
          .map((e) => RoughProcessDaysModel(srno: e.key + 1,
          stockTypeCode: e.value.stockTypeCode,
          stockTypeName: e.value.stockTypeName, days: e.value.days))
          .toList();
      _syncProcessDaysGrid();
  ...
  void _syncProcessDaysGrid() {
    _processDaysEntryData = _processDaysRows.map((r) => {
      'srno': r.srno?.toString() ?? '',
      'stockTypeCode': r.stockTypeCode?.toString() ?? '',
      'stockTypeName': r.stockTypeName ?? _stockTypeName(r.stockTypeCode?.toString()),
  ...
  Future<void> _onDelete() async {
    ...
    final confirm = await ErpDeleteDialog.show(
      context: context, theme: _theme,
      title: 'Rough Entry', itemName: 'JNO: ${_selectedRough!.jno ?? ''}',
    );
    if (confirm != true || !mounted) return;
    final success = await context.read<RoughProvider>().deleteRough(_selectedRough!.roughMstID!);
    if (success && mounted) {
  ...
  Widget _buildErpForm(BuildContext context) {
    ...
    return ErpForm(
      ...
      // Header mein extra buttons (backup: agar Enter se add nahi hua)
      // extraActions: [
      //   IconButton(
      //     tooltip: 'Add Charni',
      //     icon: const Icon(Icons.add_circle_outline, color: Colors.white, size: 18),
  ...
  Widget _buildTable(RoughProvider provider) {
    final partyNames = Map.fromEntries(
      context.read<PartyProvider>().list
          .map((e) => MapEntry(e.partyCode?.toString() ?? '', e.partyName ?? '')),
    );
    final roughTypeNames = Map.fromEntries(
      context.read<RoughTypeProvider>().roughTypes
          .map((e) => MapEntry(e.roughTypeCode?.toString() ?? '', e.roughTypeName ?? '')),
    );
    final articleNames = Map.fromEntries(
      context.read<ArticleProvider>().list
          .map((e) => MapEntry(e.articalCode?.toString() ?? '', e.articalName ?? '')),
    );
    final jangadCharniNames = Map.fromEntries(
      context.read<JangadCharaniProvider>().list
          .map((e) => MapEntry(e.jangadCharniCode?.toString() ?? '', e.jangadCharniName ?? '')),
    );
    return ErpDataTable(
      isReportRow: false,
      ...
      searchFields: const [
        ...
        // ErpSearchFieldConfig(key: 'partyCode',        label: 'PARTY',      width: 180),
        // ErpSearchFieldConfig(key: 'site',             label: 'SITE',       width: 120),
        // ErpSearchFieldConfig(key: 'jno',              label: 'JNO',        width: 100),
        // ErpSearchFieldConfig(key: 'roughTypeCode',    label: 'TYPE',       width: 140),
        // ErpSearchFieldConfig(key: 'articalCode',      label: 'ARTICAL',    width: 140),
      ],
      // searchFields: const [
      //   ErpSearchFieldConfig(key: 'jno', label: 'JNO',   hint: 'Search Jangad...', flex: 1),
      //   ErpSearchFieldConfig(key: 'kapanNo',          label: 'KAPAN NO',    hint: 'Search Kapan...',  flex: 1),
      //   ErpSearchFieldConfig(key: 'inv',              label: 'INVOICE',     hint: 'Search Invoice...', flex: 1),
      // ],
      showSearch: true,
      showFooterTotals: false,
      selectedRow: _selectedRow,
