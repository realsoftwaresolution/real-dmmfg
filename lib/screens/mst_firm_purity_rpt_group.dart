...
class MstPurityRptGroup extends StatefulWidget {
  const MstPurityRptGroup({super.key});

  @override
  State<MstPurityRptGroup> createState() => _MstPurityRptGroupState();
}

class _MstPurityRptGroupState extends State<MstPurityRptGroup> {
  // ── Theme ─────────────────────────────────────────────────────────────────
  ...
  List<ErpColumnConfig> get _tableColumns => [
    ErpColumnConfig(key: 'purityRptGroupCode', label: 'CODE', width: 130),
    ErpColumnConfig(key: 'purityRptGroupName', label: 'NAME', width: 150),
    // ErpColumnConfig(key: 'rate', label: 'RATE', width: 130),
    // ErpColumnConfig(key: 'per', label: 'PER', width: 130),
  ...
  void _onRowTap(Map<String, dynamic> row) {
    final raw = row['_raw'] as PurityRptGroupModel;
  ...
  Future<void> _onSave(Map<String, dynamic> values) async {
    final exists = await _checkDuplicate(
      fields: {
        'PurityRptGroupName': values['purityRptGroupName'],
      },
    );
    if (exists) return;
    final provider = context.read<PurityRptGroupProvider>();
    ...
    if (_isEditMode && _selectedRow != null) {
      final raw = _selectedRow!['_raw'] as PurityRptGroupModel;
      success = await provider.update(raw.purityRptGroupCode!, values);
    } else {
    ...
    if (success) {
      ...
      await ErpResultDialog.showSuccess(
        ...
        message: _isEditMode
            ? 'Purity Rpt Group updated successfully.'
            : 'Purity Rpt Group saved successfully.',
      );
  ...
  Future<void> _onDelete() async {
    final raw = _selectedRow?['_raw'] as PurityRptGroupModel?;
    if (raw?.purityRptGroupCode == null) return;
    final confirm = await ErpDeleteDialog.show(context: context, theme: _theme, title: 'Purity Rpt Group', itemName: raw!.purityRptGroupName??"");
    ...
    //     content: Text(
    //         'Delete "${raw!.purityRptGroupName}"? This cannot be undone.'),
    //     actions: [
    //       TextButton(
    ...
    if (success && mounted) {
      ...
      await ErpResultDialog.showDeleted(
        context: context,
        theme: _theme,
        itemName: raw.purityRptGroupName ?? '',
      );
      // ScaffoldMessenger.of(context).showSnackBar(
      //   SnackBar(
      //     content: const Text('Purity Rpt Group deleted successfully'),
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<PurityRptGroupProvider>(
        ...
        return Padding(
          ...
          child:
              Responsive.isMobile(context)?_showTableOnMobile?ErpDataTable(
                ...
                key: _erpFormKey,
                title: 'PURITY RPT GROUP MASTER',
                subtitle: 'Purity Rpt Group Information',
                initialTabIndex: 0,
