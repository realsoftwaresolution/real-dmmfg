...
class TrnJobWorkIssueEntry extends StatefulWidget {
  const TrnJobWorkIssueEntry({super.key});

  @override
  State<TrnJobWorkIssueEntry> createState() => _TrnJobWorkIssueEntryState();
}

class _TrnJobWorkIssueEntryState extends State<TrnJobWorkIssueEntry> {
  // ── Theme ──────────────────────────────────────────────────────────────────
  ...
  // ── Detail rows ────────────────────────────────────────────────────────────
  List<JobWorkIssueDetModel> _detRows = [];
  ...
  Future<void> _onBCodeScanned(String bCode) async {
    ...
    final rows = await context.read<JobWorkIssueEntryProvider>().fetchByBCode(
      bCode: bCode,
    );
    ...
    final newRow = JobWorkIssueDetModel(
      ...
      // IDs
      polishCheckerIssMstID: r.polishCheckerIssMstID,
      ...
      // Transaction Details
      jobWorkIssDate: r.jobWorkIssDate,
  ...
  dynamic _deleteDetRow(int idx) async {
    ...
    if (_isEditMode) {
      final confirm = await ErpDeleteDialog.show(
        ...
        itemName: 'ID: ${_detRows[actualIdx].jobWorkIssDetID?.toString()}',
      );
      if (confirm != true || !mounted) return;

      final success = await context.read<JobWorkIssueEntryProvider>().deleteRow(
        _detRows[actualIdx].jobWorkIssMstID ?? 0,
        _detRows[actualIdx].jobWorkIssDetID ?? 0,
  ...
  Future<void> _onSave(Map<String, dynamic> values) async {
    ...
    final payload = {
      "JobWorkIssDate": toUtcIso(_formValues['date']),
    ...
    bool success;
    if (_isEditMode) {
      final mstID = int.tryParse(_formValues['jobWorkIssMstID'] ?? '0') ?? 0;
    ...
    if (success) {
      ...
      await ErpResultDialog.showSuccess(
        ...
        message: wasEdit
            ? 'Job Work Issue updated successfully.'
            : 'Job Work Issue saved successfully.',
      );
      _resetForm();
      await prov.load();
    }
  ...
  Future<void> _onDelete() async {
    if (_formValues['jobWorkIssMstID'] == null ||
        _formValues['jobWorkIssMstID'] == '0')
      return;
    ...
    final confirm = await ErpDeleteDialog.show(
      ...
      theme: _theme,
      title: 'Job Work Issue',
      itemName: 'ID: ${_formValues['jobWorkIssMstID'].toString()}',
    );
    if (confirm != true || !mounted) return;

    final mstID = int.tryParse(_formValues['jobWorkIssMstID'] ?? '0') ?? 0;
    final success = await context.read<JobWorkIssueEntryProvider>().delete(
      mstID,
      theme: _theme,
      context: context,
      bCodeArray:  _detRows
    ...
    if (success && mounted) {
      final id = _formValues['jobWorkIssMstID'].toString();
      _resetForm();
      await ErpResultDialog.showDeleted(
        context: context,
        theme: _theme,
        itemName: 'Job Work Issue $id',
      );
    }
  }
  ...
  List<List<ErpFieldConfig>> _buildFormRows() {
    ...
    final processItems = deptProcessProvider.list
        .where((p) => p.active == true && p.deptCode == _selectedDeptCode)
        .map(
          (e) => ErpDropdownItem(
  ...
  Widget build(BuildContext context) {
    return Consumer<JobWorkIssueEntryProvider>(
  ...
  Map<String, String> _buildFooterTotals() {
    double sumDouble(double Function(JobWorkIssueDetModel) fn) =>
        _detRows.fold(0.0, (s, r) => s + fn(r));

    int sumInt(int Function(JobWorkIssueDetModel) fn) =>
