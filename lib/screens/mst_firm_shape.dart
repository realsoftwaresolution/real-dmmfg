...
class MstShape extends StatefulWidget {
  const MstShape({super.key});

  @override
  State<MstShape> createState() => _MstShapeState();
}

class _MstShapeState extends State<MstShape> {
  // ── Theme ─────────────────────────────────────────────────────────────────
  ...
  List<List<ErpFieldConfig>> _formRows(
      ...
      ShapeGroupProvider shapeGroupProvider,
      ) =>
      [
        /// ── BASIC INFO ──
        // [
          ...
          ErpFieldConfig(
            ...
            dropdownItems: shapeGroupProvider.list
              ...
              return ErpDropdownItem(
                label: e.shapeGroupName ?? '',
                value: e.shapeGroupCode?.toString() ?? '',
              );
            }).toList(),
            sectionIndex: 0,
          ),
  ...
  void _onRowTap(Map<String, dynamic> row) {
    final raw = row['_raw'] as ShapeModel;
  ...
  Future<void> _onSave(Map<String, dynamic> values) async {
    ...
    if (_isEditMode && _selectedRow != null) {
      final raw = _selectedRow!['_raw'] as ShapeModel;
      success = await provider.update(raw.shapeCode!, values);
    } else {
  ...
  Future<void> _onDelete() async {
    final raw = _selectedRow?['_raw'] as ShapeModel?;
    if (raw?.shapeCode == null) return;
    final confirm = await ErpDeleteDialog.show(context: context, theme: _theme, title: 'Shape', itemName: raw!.shapeName??"");
    ...
    //     content:
    //     Text('Delete "${raw!.shapeName}"? This cannot be undone.'),
    //     actions: [
    //       TextButton(
    ...
    if (success && mounted) {
      ...
      await ErpResultDialog.showDeleted(
        context: context,
        theme: _theme,
        itemName: raw.shapeName ?? '',
      );
      // ScaffoldMessenger.of(context).showSnackBar(
      //   SnackBar(
      //     content: const Text('Shape deleted successfully'),
  ...
  Widget build(BuildContext context) {
    ...
    final shapeGroupProvider = context.watch<ShapeGroupProvider>();
        ...
        return Padding(
          ...
          child:
              Responsive.isMobile(context)?_showTableOnMobile?ErpDataTable(
                ...
                rows: _formRows(companyProvider, shapeGroupProvider),
                // theme: _theme,
