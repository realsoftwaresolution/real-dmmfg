...
class MstShapeGroup extends StatefulWidget {
  const MstShapeGroup({super.key});

  @override
  State<MstShapeGroup> createState() => _MstShapeGroupState();
}

class _MstShapeGroupState extends State<MstShapeGroup> {
  // ── Theme ─────────────────────────────────────────────────────────────────
  ...
  List<ErpColumnConfig> get _tableColumns => [
    ErpColumnConfig(key: 'shapeGroupCode', label: 'CODE', width: 130),
    ErpColumnConfig(key: 'shapeGroupName', label: 'NAME', width: 220),
  ...
  void _onRowTap(Map<String, dynamic> row) {
    final raw = row['_raw'] as ShapeGroupModel;
  ...
  Future<void> _onSave(Map<String, dynamic> values) async {
    ...
    if (_isEditMode && _selectedRow != null) {
      final raw = _selectedRow!['_raw'] as ShapeGroupModel;
      success = await provider.update(raw.shapeGroupCode!, values);
    } else {
    ...
    if (success) {
      ...
      await ErpResultDialog.showSuccess(
        ...
        message: _isEditMode
            ? 'ShapeGroup updated successfully.'
            : 'ShapeGroup saved successfully.',
      );
      // ScaffoldMessenger.of(context).showSnackBar(
      //   SnackBar(
      //     content: Text(_isEditMode
      //         ? 'Shape Group updated successfully'
      //         : 'Shape Group saved successfully'),
  ...
  Future<void> _onDelete() async {
    final raw = _selectedRow?['_raw'] as ShapeGroupModel?;
    if (raw?.shapeGroupCode == null) return;
    final confirm = await ErpDeleteDialog.show(context: context, theme: _theme, title: 'Shape Group', itemName: raw!.shapeGroupName??"");

    // final confirm = await showDialog<bool>(
    //   context: context,
    //   builder: (_) => AlertDialog(
    //     title: const Text('Delete Shape Group'),
    //     content:
    //     Text('Delete "${raw!.shapeGroupName}"? This cannot be undone.'),
    //     actions: [
    //       TextButton(
    ...
    if (success && mounted) {
      ...
      await ErpResultDialog.showDeleted(
        context: context,
        theme: _theme,
        itemName: raw.shapeGroupName ?? '',
      );
      // ScaffoldMessenger.of(context).showSnackBar(
      //   SnackBar(
      //     content: const Text('Shape Group deleted successfully'),
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<ShapeGroupProvider>(
        ...
        return Padding(
          ...
          child:
              Responsive.isMobile(context)?_showTableOnMobile?ErpDataTable(
                ...
                key: _erpFormKey,
                title: 'SHAPE GROUP MASTER',
                subtitle: 'Shape Group Information',
                initialTabIndex: 0,
