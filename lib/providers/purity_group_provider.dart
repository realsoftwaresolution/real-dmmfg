...
class PurityGroupProvider extends BaseProvider {
  List<PurityGroupModel> _list = [];
  bool _isLoaded = false;

  List<PurityGroupModel> get list => _list;
  ...
  Future<void> load() async {
    final result = await request<List<PurityGroupModel>>(
      showLoader: true,
      call: () => api.get('/purity-group'),
      onSuccess: (res) {
        final data = res.data as List;
        return data.map((e) => PurityGroupModel.fromJson(e)).toList();
      },
    );
  ...
  Future<bool> create(Map<String, dynamic> formValues) async {
    ...
    final model = PurityGroupModel.fromFormValues(formValues);

    final result = await request<PurityGroupModel>(
      showLoader: true,
      call: () => api.post('/purity-group', data: model.toJson()),
      onSuccess: (res) => PurityGroupModel.fromJson(res.data),
    );
