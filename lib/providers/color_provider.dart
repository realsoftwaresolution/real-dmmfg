import 'package:rs_dashboard/rs_dashboard.dart';
import '../models/color_model.dart';
import '../models/company_model.dart' show CompanyModel;

class ColorProvider extends BaseProvider {
  List<ColorModel> _list = [];
  bool _isLoaded = false;

  List<ColorModel> get list => _list;
  bool get isLoaded => _isLoaded;

  List<CompanyModel> _companies = [];

  void setCompanies(List<CompanyModel> companies) {
    _companies = companies;
    notifyListeners();
  }
  int? _selectedCompanyCode;

  void setSelectedCompany(int? code) {
    _selectedCompanyCode = code;
  }
  List<Map<String, dynamic>> get tableData =>
      _list.map((d) {

        final company = _companies
            .where((c) => c.companyCode == d.companyCode)
            .firstOrNull;
        return d.toTableRow(companyName: company?.companyName);
      }).toList();

  // ───── LOAD ─────
  Future<void> load() async {
    final result = await request<List<ColorModel>>(
      showLoader: true,
      call: () => api.get('/color'),
      onSuccess: (res) {
        final data = res.data as List;
        final list = data.map((e) => ColorModel.fromJson(e)).toList();
        list.sort((a, b) => a.sortID!.compareTo(b.sortID!.toInt()));
        return list;
      },
    );

    if (result != null) {
      _list = result;
      _isLoaded = true;
      notifyListeners();
    }
  }

  // ───── CREATE ─────
  Future<bool> create(Map<String, dynamic> formValues) async {
    formValues['companyCode'] = _selectedCompanyCode?.toString() ?? '';

    final model = ColorModel.fromFormValues(formValues);

    final result = await request<ColorModel>(
      showLoader: true,
      call: () => api.post('/color', data: model.toJson()),
      onSuccess: (res) => ColorModel.fromJson(res.data),
    );

    if (result != null) {
      _list.insert(0, result);
      notifyListeners();
      return true;
    }
    return false;
  }

  // ───── UPDATE ─────
  Future<bool> update(int code, Map<String, dynamic> formValues) async {
    formValues['companyCode'] = _selectedCompanyCode?.toString() ?? '';

    final model = ColorModel.fromFormValues(formValues);

    final result = await request<ColorModel>(
      showLoader: true,
      call: () => api.put('/color/$code', data: model.toJson()),
      onSuccess: (res) => ColorModel.fromJson(res.data),
    );

    if (result != null) {
      final index = _list.indexWhere((e) => e.colorCode == code);
      if (index != -1) _list[index] = result;
      notifyListeners();
      return true;
    }
    return false;
  }

  // ───── DELETE ─────
  Future<bool> delete(int code) async {
    final result = await request<bool>(
      showLoader: true,
      call: () => api.delete('/color/$code'),
      onSuccess: (_) => true,
    );

    if (result == true) {
      _list.removeWhere((e) => e.colorCode == code);
      notifyListeners();
      return true;
    }
    return false;
  }
}